...
async function signup() {
    ...
    const vnPrefixes = ["03", "07", "08", "09"];
    const prefix = vnPrefixes[Math.floor(Math.random() * vnPrefixes.length)] + Math.floor(Math.random() * 10);
    const phone = prefix + String(Math.floor(Math.random() * 10_000_000)).padStart(7, "0");
